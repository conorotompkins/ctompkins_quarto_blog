...
census_pca |> 
  pivot_longer(-c(NAME, total_population)) |> 
  ggplot(aes(total_population, value)) +
  geom_bin_2d() +
  facet_wrap(vars(name), scales = "free") +
  scale_x_log10(expand = expansion(mult = c(.1, .1))) +
...
dist_thres <- min_distthreshold(census_tracts_pca)

w_dist <- distance_weights(census_tracts_pca, dist_thres, power=1.0,  is_inverse=FALSE, is_arc=FALSE, is_mile=TRUE)
...
```{r}
tract_clusters |> 
  select(total_population, income, contains("pct"), cluster_greedy, -pct_total) |> 
  st_drop_geometry() |> 
  pivot_longer(-c(cluster_greedy, total_population)) |> 
  ggplot(aes(total_population, value, color = cluster_greedy, fill = cluster_greedy)) +
  ggforce::geom_mark_hull(concavity = 10, alpha = .3) +
...
tract_clusters |> 
  ...
  summarize(total_population = mean(total_population),
            value = mean(value),
            .by = c(cluster_greedy, name)) |> 
  ggplot(aes(total_population, value, fill = cluster_greedy, label = cluster_greedy)) +
  geom_point(aes(color = cluster_greedy)) +
