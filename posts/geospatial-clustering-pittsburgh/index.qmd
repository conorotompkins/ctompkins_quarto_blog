...
census_tracts_wide <- census_tracts |> 
  ...
  mutate(across(-c(NAME, geometry, total), ~.x / total, .names = "pct_{.col}")) |> 
  rowwise() |> 
...
cluster_df <- census_tracts_wide |> 
  select(geometry, total, contains("pct"), -pct_total) |> 
  st_drop_geometry()

bound_vals <- census_tracts_wide['total']
...
labels <- sprintf(
  ...
  tract_clusters$NAME, tract_clusters$cluster_greedy, tract_clusters$pct_white, tract_clusters$pct_black, tract_clusters$pct_asian, tract_clusters$pct_hispanic, tract_clusters$total
...
cluster_pca <- tract_clusters |> 
  select(-pct_total) |> 
  st_drop_geometry() |> 
  summarize(across(c(contains("pct"), total), mean),
            .by = cluster_greedy)

cluster_pca |> 
  pivot_longer(-c(cluster_greedy, total)) |> 
  ggplot(aes(total, value, label = cluster_greedy)) +
  geom_label() +
