...
min_bound <- census_tracts_wide |> 
  ...
  mutate(min_bound = total_population * .08) |> 
  pull(min_bound)
...
labels <- sprintf(
  ...
  tract_clusters$NAME, tract_clusters$cluster_greedy, tract_clusters$pct_white, tract_clusters$pct_black, tract_clusters$pct_asian, tract_clusters$pct_hispanic, tract_clusters$total_population
...
cluster_pca <- tract_clusters |> 
  select(-pct_total) |> 
  st_drop_geometry() |> 
  summarize(across(c(contains("pct"), total_population), mean),
            .by = cluster_greedy)
