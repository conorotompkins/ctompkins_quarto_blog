...
tract_od_directions |> 
  distinct(h_tract_geo, w_tract_geo) |> 
...
home_address <- tract_od_directions |> 
  distinct(home_address, h_tract,  h_tract_geo, home_address_location_geocoded) |> 
  mutate(home_x = map_dbl(home_address_location_geocoded, 1),
         home_y = map_dbl(home_address_location_geocoded, 2)) |> 
  select(-home_address_location_geocoded) |> 
  left_join(allegheny_county_tracts, by = c("h_tract" = "GEOID"))

st_geometry(home_address) <- "h_tract_geo"

home_address |> 
  filter(home_address == "Penn-Lincoln Parkway East, Swissvale, Pennsylvania 15205, United States") |> 
  st_sf() |> 
...
tract_od_directions |> 
  ...
  mutate(home_x = map_dbl(home_address_location_geocoded, 1),
         home_y = map_dbl(home_address_location_geocoded, 2)) |> 
  mutate(work_x = map_dbl(work_address_location_geocoded, 1),
         work_y = map_dbl(work_address_location_geocoded, 2)) |> 
  ggplot() +
  geom_sf(data = allegheny_county_tracts) +
  geom_sf(data = allegheny_county_tracts |> filter(GEOID == "42003515401"), fill = "blue") + 
  geom_sf() +
  geom_point(aes(home_x, home_y), color = "blue") +
  geom_point(aes(work_x, work_y), color = "red")

allegheny_county_tracts |> 
  leaflet::leaflet() |> 
  leaflet::addPolygons(weight = .1) |> 
  leaflet::addPolygons(data = allegheny_county_tracts |> filter(GEOID == "42003515401"), fillColor = "red", fillOpacity = 1)

address_work <- tract_od_directions |> 
  distinct(work_address_location_geocoded)

address_work |> 
  mutate(x = map_dbl(work_address_location_geocoded, 1),
         y = map_dbl(work_address_location_geocoded, 2)) |> 
  ggplot() +
  geom_sf(data = allegheny_county_tracts) +
  geom_point(aes(x, y))
...
tract_od_directions |> 
  st_drop_geometry() |> 
  distinct(home_address, h_tract_geo) |> 
  arrange(desc(h_tract_geo))

tract_od_directions |> 
  st_drop_geometry() |> 
  distinct(work_address, w_tract_geo) |> 
  arrange(w_tract_geo) |> 
  view()

tract_od_directions |> 
  distinct(home_address)
```
