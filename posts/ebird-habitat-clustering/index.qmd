...
```{r}
test_sp <- c("Dark-eyed Junco", "Field Sparrow", "Yellow Warbler", "Snow Goose", "American Crow")

bird_df |> 
  filter(common_name == "Snow Goose") |> 
  mutate(log10 = log10(prop_pop_max + 1)) |> 
  ggplot(aes(prop_pop_max, log10)) +
  geom_bin_2d()

bird_df |> 
  select(common_name, prop_pop_max) |> 
  filter(common_name == test_sp[5]) |> 
  mutate(log10 = log10(prop_pop_max + 1),
         log = log(prop_pop_max + 1)) |> 
  pivot_longer(cols = -common_name) |> 
  ggplot(aes(value, fill = name)) +
  geom_density(alpha = .5) +
  facet_wrap(vars(name), scale = "free", ncol = 1)
```


```{r calc metrics}
bird_df_scale_compare <- bird_df |> 
...
bird_df_scale_compare <- bird_df_scale_compare |> 
  ...
  select(common_name, x, y, prop_pop_max, scaled, rescaled, log, log10) |> 
  pivot_longer(cols = c(prop_pop_max, scaled, rescaled, log, log10))
...
test_df <- bird_df_scale_compare |> 
  filter(common_name == test_sp[1])
...
PC1 is the "Deep Forest" index. Higher values indicate that the bird prefers deeper forest habitat. PC2 indicates how open the habitat is. Higher values indicate that the bird prefers habitat with less forest.
...
PC1 is a continuum of deep forest vs water. The Susquehanna River (southeast) and Ohio River (southwest) are highlighted.
```{r}
tile_df |> 
...
PC2 indicates presence/absence of farmland. The area where the Susquehanna River bisects the Allegheny Mountains is known for its farmland.
```{r}
tile_df |> 
  left_join(tile_pca_rot) |> 
...
PC3 indicates the steepness of mountains.
```{r}
tile_df |> 
  left_join(tile_pca_rot) |> 
