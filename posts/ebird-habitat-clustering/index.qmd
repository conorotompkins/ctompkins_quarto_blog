...
For this analysis I focus on the proportion of the population for a given species that exist in a 3x3 kilometer area. I focus on migratory birds that the Cornell team has high confidence estimates for.
...
```{r read data}
# bird_df_raw <- read_csv("posts/ebird-habitat-clustering/post_data/species_abundance_present_in_pa.csv") |>
...
These transformations are not very effective with this type of skew. Using a scale function to transform the data relative to its mean is not effective for zero-inflated data because the mean is very close to the lower limit (0) of the data.
...
I ended up choosing the `scales::rescale` transformation because the result looks marginally better inthe above maps, but more testing would be needed to really justify this choice.
```{r}
bird_df <- bird_df_raw |> 
  mutate(rescaled = rescale(prop_pop_max, to = c(-1, 1)),
         .by = common_name) |> 
  select(common_name, x, y, rescaled)

bird_df_wide <- bird_df |> 
  pivot_wider(names_from = common_name, values_from = rescaled)

bird_df_wide_noloc <- select(bird_df_wide, -c(x, y))
```

The Snow Goose in Pennsylvania is a good example of highly zero-inflated abundance distribution because it is either absent or hugely abundant, and only in specific areas.
...
<iframe src="https://macaulaylibrary.org/asset/290513131/embed" height="509" width="640" frameborder="0" allowfullscreen></iframe>
...
`tot.withinss` stops decreasing around 3 clusters, which means that the clusters stop becoming significantly more compact after 3 clusters.

This code fits the kmeans algorithm against the data using 3 clusters.
...
-   Cluster 4 is a unique area in southeastern Pennsylvania that attracts very large flocks of blackbirds. This may also contain the Middle Creek WMA, which is a critical stopover point for Snow Goose during their migration.
...
global_avg <- bird_df |> 
  summarize(prop_pop_global_mean = mean(rescaled),
            .by = c(common_name))
```
...
This shows the same dendrogram cut at 5 clusters (branches).

```{r hclust 5}
tree_cut <- hclust_tree |> cut(h = 600)
...
* Cluster 1 indicates areas with more open habitat and less water. This area is notably smaller compared to the same habitat cluster from the kmeans algorithm.
...
The kmeans clusters have a higher silhouette score and a lower SSE ratio, which means that they are more distinct and the observations within each cluster are more similar to each other than to observations from other clusters. The hierarchical method produced clusters that are less distinctive and less internally cohesive.
...
sampled_sp <- bird_df_raw |>
  select(common_name, prop_pop_max) |>
  summarize(mean = mean(prop_pop_max), .by = common_name) |>
...
This is not a feasible analysis tool at scale, but it confirms some basic intuitive findings. For example, Ovenbird and Scarlet Tanager are highly correlated.

## PCA

### On birds

This code calculates principal components using each species' rescaled `prop_pop_max` as inputs.
...
PC1 is the "Deep Forest" index. Higher values indicate that the bird prefers deeper forest habitat. PC2 indicates how open the canopy of the habitat is. Higher values indicate that the bird prefers habitat with less forest.
...
tile_pca_fit <- bird_df |> 
  left_join(tile_df) |> 
  select(tile_id, common_name, rescaled) |> 
  pivot_wider(names_from = tile_id, values_from = rescaled) |> 
  select(-common_name) |> 
  prcomp(scale = TRUE)
...
PC3 is not an obvious indicator. It may show whether the habitat drains into the Mississippi or Atlantic watershed.
