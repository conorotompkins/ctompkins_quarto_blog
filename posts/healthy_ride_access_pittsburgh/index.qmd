...
stations <- read_csv("posts/healthy_ride_access_pittsburgh/post_data/stations/healthy-ride-station-locations-2020-q2.csv",
                     col_types = cols(
...
city_boundary <- st_read("posts/healthy_ride_access_pittsburgh/post_data/Pittsburgh_City_Boundary-shp/City_Boundary.shp",
                         quiet = TRUE) %>% 
  filter(FID != 7) %>% 
  summarize() %>% 
  st_boundary() %>% 
...
station_isochrone <- st_read("posts/healthy_ride_access_pittsburgh/post_data/stations/station_isochrone/station_isochrone.shp",
                             quiet = TRUE)
...
```{r include = FALSE}
station_isochrone <- st_read("post_data/stations/station_isochrone/station_isochrone.shp",
                             quiet = TRUE)
```
