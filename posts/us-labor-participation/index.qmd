...
Here I use the `tsibble` package to transform the dataframe into a time series table. Then I plot the data with `autoplot`.
...
-   Seasonality
    ...
    -   The strong seasonailty among men peaks early in this series becomes weaker over time.

    -   The pattern of seasonality among women appears to be somewhat strong but unstable early on, and then becomes weaker over time.

    -   My hypothesis is that the decreasing seasonality is due to changes in what jobs are available and people's preference for those jobs. For example, agricultural labor and construction are highly seasonal, but may have become a smaller % of the available jobs over time.
...
Here I use STL to decompose the time series into trend, seasonality, and remainder. This models the strength of the trend and seasonality over time. The remainder is the noise in the data that cannot be explained by the trend or seasonality. I use `components` to extract the components of the decomposition to analyze them further.
...
The decomposition for men shows that the magnitude of the seasonality shrinks over time, and the pattern in which months are peaks and troughs changes as well. The `season_year` variable represents the seasonality.
...
The seasonality pattern for women changes drastically multiple times over the course of the time series. The grey bars on the left indicate the reverse magnitude of the effect. The height of the grey bars for seasonality and the remainder are similar for women, so the seasonal effect is smaller overall.
...
The LFPR trend for women actually increased at the height of the 2008 financial crisis. This could indicate that some women were pulled into the labor force in response to increased economic pressure on their household.

This shows the de-trended seasonality at different scales for each group:

```{r}
x <- stl_components |> 
  ggplot(aes(date, season_year, color = series, group = series)) +
  geom_line() +
...
There are clearly changes in seasonality over time, specifically in periods when the magnitude of the seasonality is weaker.
...
Both series have very strong trends, but the series for men has stronger seasonality. This reflects my impressions from the graphs above. The series have very similar low `spectral_entropy` values which shows that there is very little noise in the data. I would expect both series to be easily forecastable.
...
This calculates the starting year-month of each time period and calculates the features for each.
...
```{r}
fred_tile_features |> 
  ggplot(aes(trend_strength, seasonal_strength_year, fill = series, label = period_starting)) +
  geom_label() +
...
month_seasonality |> 
  count(series, seasonal_trough_year) |> 
  arrange(series, desc(n))
```
...
month_seasonality |> 
  ...
  complete(series, period_starting, name, value = month.abb) |> 
  mutate(flag = !is.na(n)) |> 
...
month_seasonality |> 
  ...
  complete(series, period_starting, name, value = month.abb) |> 
  mutate(value = fct_relevel(value, month.abb)) |> 
  mutate(flag = !is.na(n),
         value = fct_rev(value)) |> 
  ggplot(aes(period_starting, value, fill = flag)) +
  geom_tile() +
...
stl_components |> 
  ...
  scale_fill_viridis_c(name = "Seasonal strength") +
  theme(legend.position = "bottom")
```

The seasonality among men appears to have started diffusely in the late summer, concentrated in the same season in the late 1960s, and then diffused again over time.
...
seasonal_model |> 
  ...
  scale_fill_viridis_c(name = "Seasonal strength") +
  facet_wrap(vars(series), ncol = 1) +
  labs(subtitle = "January is reference month") +
  theme(legend.position = "bottom")
```

Here I apply Principal Component Analysis to the features calculated on the tiled time series. The most recent period for each series, which contains the COVID-19 pandemic, is an outlier along the second component.
...
pcs |>
  ...
  geom_point() +
  geom_label(aes(label = graph_label), nudge_x = -5, nudge_y = 1, size = 3) +
...
The second component is positively correlated with the seasonality of the series (`seasonal_strength_year`) and negatively correlated with the amount of noise in the series (`spectral_entropy`).
...
This makes sense because of the significant decrease in seasonality during this time period.
