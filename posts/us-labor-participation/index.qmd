...
-   Seasonality
    ...
    -   The pattern of seasonality among women appears to be somewhat strong but highly variable early on, and then becomes weaker over time.
...
The decomposition for men shows that the magnitude of the seasonality shrinks over time, and the pattern in which months are peaks and troughs changes as well. The `season_year` variable represents the seasonality. The grey bars on the left indicate the reverse magnitude of the effect. Among men, the `remainder` seems to have a stronger effect than seasonality (`seasonal_year`), though that is probably skewed by the singularly large outlier value of the COVID-19 pandemic.
...
seasonal_model |> 
  ...
  scale_y_discrete(expand = c(0, 0)) +
  scale_fill_viridis_c(name = "Seasonal strength") +
  labs(subtitle = "January is hidden reference month") +
  theme(legend.position = "bottom",
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())
```
...
pcs |>
  ...
  geom_point() +
  ggrepel::geom_label_repel(aes(label = graph_label), size = 3) +
