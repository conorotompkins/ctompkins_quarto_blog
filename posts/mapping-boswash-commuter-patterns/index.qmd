...
tags: [Census, Commuters]
categories: [Census, Commuters]
date: 2020-09-25
...
#| eval: false
node_pos |> 
  write_csv("post_data/node_pos.csv")
```

```{r}
#| echo: false
read_csv("post_data/node_pos.csv") %>% 
  glimpse()
```
...
#| eval: false
nodes |> 
  write_csv("post_data/nodes.csv")
```

```{r}
#| echo: false
read_csv("post_data/nodes.csv") %>% 
  glimpse()
```
...
#| eval: false
edges |> 
  write_csv("post_data/edges.csv")
```

```{r}
#| echo: false
read_csv("post_data/edges.csv") %>% 
  glimpse()
```
