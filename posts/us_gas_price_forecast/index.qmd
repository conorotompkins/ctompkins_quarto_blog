...
This creates a test set of the last 104 weeks of each series, and a corresponding training set of the data before that time period. then I train models of various structures, which I will then forecast and find the accuracy to determine which model structure is most appropriate.
```{r}
oil_prices |>
  count(series)
...
```{r}
recent_history <- oil_prices |> group_by(series) |> slice_tail(n = 60)
...
test_fc |>
  filter(series == "brent_crude") |>
  mutate(.model = factor(.model, levels = bc_acc)) |>
  autoplot(recent_history) +
...
models_cv_exo <- ts_cv |>
  model(
    ...
    dhr7_pdq_bc_lag1 = ARIMA(
      log(price + 1) ~ fourier(period = 365.25 / 7, K = 7) +
        PDQ(0, 0, 0) +
        brent_crude_lag1
    ),
    dhr7_pdq_bc_lag5 = ARIMA(
      log(price + 1) ~ fourier(period = 365.25 / 7, K = 7) +
        PDQ(0, 0, 0) +
        brent_crude_lag5
    ),
    dhr7_pdq_bc_lag10 = ARIMA(
      log(price + 1) ~ fourier(period = 365.25 / 7, K = 7) +
        PDQ(0, 0, 0) +
        brent_crude_lag10
    ),
    ...
    dhr7_pdq_bc_mean30 = ARIMA(
      log(price + 1) ~ fourier(period = 365.25 / 7, K = 7) +
        PDQ(0, 0, 0) +
        brent_crude_mean30
    ),
    dhr7_pdq_bc_mean60 = ARIMA(
      log(price + 1) ~ fourier(period = 365.25 / 7, K = 7) +
        PDQ(0, 0, 0) +
        brent_crude_mean60
    ),
    dhr7_pdq_bc_max10 = ARIMA(
      log(price + 1) ~ fourier(period = 365.25 / 7, K = 7) +
        PDQ(0, 0, 0) +
        brent_crude_max10
    ),
    dhr7_pdq_bc_max20 = ARIMA(
      log(price + 1) ~ fourier(period = 365.25 / 7, K = 7) +
        PDQ(0, 0, 0) +
        brent_crude_max20
    ),
    dhr7_pdq_bc_max30 = ARIMA(
      log(price + 1) ~ fourier(period = 365.25 / 7, K = 7) +
        PDQ(0, 0, 0) +
        brent_crude_max30
    ),
