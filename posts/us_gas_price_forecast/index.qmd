...
Here I use log(price + 1) to constrain the forecasts to be positive. I use fourier terms like `fourier(365.25 / 7, k = x)` to represent the impact of within-year weekly seasonality without having to make \~52 dummy variables. K varies how "smooth" the representation is. Higher values of K fit the model closer to the variance in the data, and lower values of K use a simpler pattern.
...
recent_history <- us_oil_prices |>
  semi_join(fc_index)

# ts_cv is your full original tsibble (actuals)
ggplot() +
  # full actuals in every facet
  autolayer(oil_prices) +
  # CV forecasts (one slice, faceted by model)
  autolayer(
    test_fc |>
      filter(.id == max(.id)) |>
      mutate(.model = factor(.model, levels = us_acc)),
  ...
  facet_wrap(vars(.model, .id), scales = "free_y") +
  guides(
    fill = "none",
    color = "none",
    fill_ramp = guide_legend(title = "Prediction interval")
...
acc_by_h_id |>
  ...
  group_by(.model, h) |>
  summarise(
    lower = quantile(RMSE, 0.25, na.rm = TRUE),
    median = median(RMSE, na.rm = TRUE),
    upper = quantile(RMSE, 0.75, na.rm = TRUE)
  ) |>
  ggplot(aes(x = h, y = median, color = .model)) +
  geom_ribbon(
  ...
  labs(
    ...
    subtitle = "Median and 25/75th percentiles by forecast horizon",
    x = "Forecast horizon (weeks)",
    y = "RMSE"
  ) +
...
The two models have similar error trajectories over the first 8 weeks, but significantly diverage over the next 20 weeks. The error for `dcmp` increases much more rapidly than `dhr5_free`. Based on RMSE, 12 weeks is a reasonable forecast horizon for `dhr5_free`.

Interestingly, the error curve for `dcmp` continues to increase after week \~20, while `dhr5_free` stabilizes. Then the error curve for `dcmp` decreases after week \~32. The variance in the error of `dhr5_free` increases, as indicated by the wider IQR in the boxplot. This variance is driven by outliers from one cross-validation forecast. Further cross-validation could help smooth that out, but I don't have the compute power for that at hand.

My hypothesis for this is that in the short-term, US oil price is driven by events, but long-term variance is driven by the global oil market, the push and pull of supply and demand, and long-term structural aspects of the global economy. It may be easier for a model without exogenous variables to forecast what the price of oil will be in 52 weeks than in 32 weeks.
...
The model (`{r} best_model`) that uses a 10-week moving average of Brent Crude Oil as a predictor has the best `crps_skill`. `{r} best_model` provides a ~5 percentage point increase in accuracy over the "vanilla" `dhr_k5_free` model, while most of the other models with exogenous variables do worse than `dhr_k5_free`.
...
cv_fc |>
  ...
  facet_wrap(vars(.id, .model)) +
  scale_y_continuous(label = \(x) str_c("$", x)) +
  guides(fill_ramp = guide_legend(title = "Prediction interval")) +
  labs(
    title = "Cross-validated forecast",
...
This model forecasts that US oil prices will increase for the next \~7 weeks regardless of the scenario. The high end of the "Increase" scenario is \> \$5.50. Prices remain above \$4 per gallon in the average outcome for all scenarios. This is likely because the global oil supply chain has already been severely constrained, and even in a best case scenario it would take weeks for oil to transit from its origin to consumer markets.

#### Future extension

A possible improvement on this method is to include the release of oil from the IEA (International Energy Agency) oil reserves. Oil is released from these reserves to reduce pressure on supply, which should decrease the price. This function is similar to a stock market circuit-breaker (link) or time-outs in basketball (link).