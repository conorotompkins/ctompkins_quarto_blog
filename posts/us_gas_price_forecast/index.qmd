...
In this post, I analyze the relationship between the [Brent Crude Oil index](https://fred.stlouisfed.org/series/DCOILBRENTEU) and US gas prices at the pump [GASREGW](https://fred.stlouisfed.org/series/GASREGW/) (BC). I use time series forecasting with exogenous variables to test whether BC is a useful predictor of the price of gas in the US.
...
A basic scatterplot of the WoW % change of the two series shows that there is not a strong relationship.
...
This code decomposes the series into time series components and plots them. The 
...
The model (`{r} best_model`) that uses a 10-week moving average of BC as a predictor has the best `crps_skill`. `{r} best_model` provides a ~5 percentage point increase in accuracy over the "vanilla" `dhr_k5_free` model, while most of the other models with exogenous variables do worse than `dhr_k5_free`.
...
Then I set up some future scenarios of Brent Crude to use as predictors. I create the following 12-week scenarios:
...
A possible improvement on this method is to include the release of oil from the IEA (International Energy Agency) oil reserves. Oil is released from these reserves to reduce pressure on supply, which should decrease the price. This function is similar to a [stock market circuit-breaker](https://www.investopedia.com/terms/c/circuitbreaker.asp) or [time-outs in basketball](https://arxiv.org/abs/2011.11691).