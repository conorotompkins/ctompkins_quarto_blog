...
df_ts |> 
  write_csv("post_data/summarized_parking_data.csv")
```

```{r}
#| echo: false

df_ts <- read_csv("post_data/summarized_parking_data.csv") %>% 
  group_by(year, week_of_year) %>%
  mutate(first_date_of_week = min(start_date)) %>% 
  ungroup() %>% 
  #select(-start_date) %>% 
...
#| eval: false
#| echo: false
write_csv(df_ts_neighborhood, "post_data/summarized_parking_data_neighborhood.csv")
```

```{r}
#| echo: false
df_ts_neighborhood <- read_csv("post_data/summarized_parking_data_neighborhood.csv") %>% 
  group_by(year, week_of_year) %>% 
  mutate(first_date_of_week = min(start_date)) %>% 
  ungroup() %>% 
